...
### HarmonyOS target, app title, and icon

HarmonyOS **launcher label** and **icon** are not derived from Flutter `app.name` automatically. Configure them under `ohos.target.source.abilities` for `EntryAbility`, as in [example/flavorizr.yaml](example/flavorizr.yaml). This aligns with Huawei’s multi-product / multi-target model ([custom targets and products](https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-customized-multi-targets-and-products-guides#section82111917125413)).

| Field | Purpose |
|:------|:--------|
| **label** | Use `"$string:resource_name"` to reference a string in `resources/.../element/string.json`. For example `"$string:favorite_app_name"` maps to a `string` entry with that `name`. Flavorizr creates or updates `string.json` under each flavor resource tree; if `value` is missing, it fills the **flavor key** (e.g. `apple`) as a placeholder—you can edit `value` to match `app.name` or your final title. |
| **icon** | Path to an image **relative to the Flutter project root** (e.g. `assets/apple_flavor_icon.png`). After `ohos:targets`, the file is copied to that flavor’s `base/media/` as `{flavorKey}_icon` plus the original extension (e.g. `apple_icon.png`), and `entry/build-profile.json5` writes **`"icon": "$media:{flavorKey}_icon"`** (no extension in the media reference, Harmony style). |

Other common `ohos.target` keys (see the example file):

| Path | Purpose |
|:-----|:--------|
| **target.source.sourceRoots** | Extra ArkTS source roots (relative to the `entry` module); directories are created if missing. |
| **target.source.pages** | Page list (e.g. `pages/Index`) merged into the target’s `source.pages`. |
| **target.output.artifactName** | HAP artifact name (e.g. `apple_debug`). |
| **target.resource.directories** | Resource roots for this target; often a flavor-specific folder plus shared `./src/main/resources`. |
...
| ohos:targets            | OHOS          | Auto-detects Harmony entry build-profile files and updates targets      |
...
| target              | Object |         | false    | Maps to `entry` `targets[]`; may include `source` (`sourceRoots`, `pages`, `abilities`), `output`, `resource`. See [HarmonyOS target, app title, and icon](#harmonyos-target-app-title-and-icon) above |
...
| signingConfig        | String | from `ohos.name`    | false    | Signing config name                         |
...
- https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/ide-customized-multi-targets-and-products-guides#section82111917125413
